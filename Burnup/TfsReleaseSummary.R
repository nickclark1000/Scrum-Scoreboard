GetCurrentRelease <- function() {
  # Computes the current release based on today's date.  Args: None
  # Returns: current.release: The current release as a list.
  if (tfs.collection == "" || tfs.project == "") 
    stop("TFS Collection or Project is not defined", call. = FALSE)
  url <- paste("/tfs/", tfs.collection, "/", tfs.project, "/_apis/wit/classificationNodes/iterations?$depth=3", 
               sep = "")
  ...
  for (i in 1:nrow(releases$children)) {
    ...
    if (!is.na(child$attributes$startDate) || !is.na(child$attributes$finishDate)) {
      cat("Release:", child$name, ", Start:", child$attributes$startDate, 
          ", Finish:", child$attributes$finishDate, "\n")
    ...
    for (i in 1:nrow(current.major.release$children[[1]])) {
      ...
      if (grandchild$hasChildren) {
        if (!is.na(grandchild$attributes$startDate) || !is.na(grandchild$attributes$finishDate)) {
          cat("Release:", grandchild$name, ", Start:", grandchild$attributes$startDate, 
              ", Finish:", grandchild$attributes$finishDate, "\n")
          if (today > grandchild$attributes$startDate && today < 
              grandchild$attributes$finishDate) {
...
GetReleaseSprints <- function(release) {
  # Returns a list of sprints in the release.  Args: release: Release
  # list.  Returns: sprints$children: List of child sprints in the
  # release.
  if (tfs.collection == "" || tfs.project == "") 
    stop("TFS Collection or Project is not defined", call. = FALSE)
  url <- paste("/tfs/", tfs.collection, "/", tfs.project, "/_apis/wit/classificationNodes/iterations/", 
               URLencode(release$name), "?$depth=1", sep = "")
...
GetReleaseWorkItemIds <- function(iteration.ids, date = format(Sys.Date())) {
  ...
  url <- paste("/tfs/", tfs.collection, "/", tfs.project, "/_apis/wit/wiql?api-version=1.0", 
               sep = "")
...
GetReleaseWorkItems <- function(work.item.id.list, date = format(Sys.Date())) {
  ...
  while (length(remainder) > 0) {
    ...
    url <- paste("/tfs/", tfs.collection, "/_apis/wit/workitems?ids=", 
                 list, "&asOf=", date, "&fields=", gsub("[\n ]", "", return.fields), 
                 "&api-version=1.0", sep = "")
...
GetBacklogSizeOverTime <- function(iteration.ids, dates) {
  cat("Dates:", dates)
  backlog.size.over.time <- data.frame(TOTAL_RELEASE_POINTS = double(), 
                                       AS_OF = character())
  for (i in 1:length(dates)) {
    work.item.ids <- GetReleaseWorkItemIds(iteration.ids, dates[i])
    work.item.df <- GetReleaseWorkItems(work.item.ids$workItems$id, 
                                        dates[i])
    backlog.size.as.of <- data.frame(TOTAL_RELEASE_POINTS = sum(work.item.df$Microsoft.VSTS.Scheduling.Effort, 
                                                                na.rm = TRUE), AS_OF = dates[i])
