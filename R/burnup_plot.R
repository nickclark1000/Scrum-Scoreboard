...
NO_CHANGE_LINE <- data.frame(x = c(as.numeric(as.POSIXct((tail(release_summary$END_DATE, 1))))*1000, 
                                   as.numeric(as.POSIXct((tail(release_summary$END_DATE, 1))))*1000 + 1000000), 
                             y = c(tail(release_summary$TOTAL_RELEASE_POINTS, 1), 
                                   tail(release_summary$TOTAL_RELEASE_POINTS, 1)))
NO_CHANGE_LM <- lm(NO_CHANGE_LINE$y ~ NO_CHANGE_LINE$x)


###Initialize data frame for 5-sprint avg velocity forecast
FORECAST_AVG_5 <- data.frame(DATE = c(as.numeric(as.POSIXct((tail(release_summary$END_DATE, 1)))), 
                                      as.numeric(as.POSIXct((tail(release_summary$END_DATE, 1)))) + 1210000), 
                             AVG_5_PROJECTED_POINTS = c(tail(release_summary$COMPLETED_RELEASE_POINTS,1), 
                                                        tail(release_summary$COMPLETED_RELEASE_POINTS,1) + mean(diff(tail(release_summary$COMPLETED_RELEASE_POINTS, 6)))))
...
FORECAST_BACKLOG_AVG_5 <- data.frame(DATE = c(as.numeric(as.POSIXct((tail(release_summary$END_DATE, 1)))), 
                                              as.numeric(as.POSIXct((tail(release_summary$END_DATE, 1)))) + 1210000), 
                                     AVG_5_PROJECTED_POINTS = c(tail(release_summary$TOTAL_RELEASE_POINTS,1), 
                                                                tail(release_summary$TOTAL_RELEASE_POINTS,1) + mean(diff(tail(release_summary$TOTAL_RELEASE_POINTS, 6)))))
...
plotBurnupChart <- function(df, targetDate, chartTitle, yAxisTitle){
  ...
  p<-plot_ly(data = long_df, x = as.Date(Date), y=Points, color = Type, fill = 'tonexty') %>%
    ...
    ##No Change Target
    add_trace(x = c(totalConstantDate, totalConstantDate), 
              y = c(0, NC_LAST5AVG_INTERSECTION$y), 
              mode = "lines", 
              name = "No Change Target", 
              line = list(dash = "dash", color = "purple"), 
              showlegend = FALSE) %>%
    ##No Change horizontal
    add_trace(x = c(lastDate, totalConstantDate), 
              y = c(NC_LAST5AVG_INTERSECTION$y, NC_LAST5AVG_INTERSECTION$y), 
              mode = "lines", 
              name = "No Change to Backlog Size", 
              line = list(dash = "dash", color = "purple")) %>%
    ##Projected Backlog Size
    add_trace(x = c(lastDate, totalIncreaseDate), 
              y = c(lastDateUnix * BACKLOG_AVG_5_LM$coef[2] + BACKLOG_AVG_5_LM$coef[1], totalIncreasePoints), 
              name = "Projected Backlog Size", 
              line = list(dash = "dash", color = "orange"), 
              showlegend = TRUE) %>% 
    ##Backlog Change Target
    add_trace(x = c(totalIncreaseDate, totalIncreaseDate), 
              y = c(0, totalIncreasePoints), 
              mode="lines", 
              line = list(dash = "dash", color = "orange"), 
              showlegend = FALSE) %>%
    ##Velocity trend
    add_trace(x = c(lastDate, max(totalIncreaseDate, totalConstantDate)), 
              y = c(lastDateUnix * AVG_5_LM$coef[2] + AVG_5_LM$coef[1], max(totalIncreasePoints, NC_LAST5AVG_INTERSECTION$y)), 
